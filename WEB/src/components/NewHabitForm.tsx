...
export function NewHabitForm() {
  ...
  async function createNewHabit(event: FormEvent) {
    ...
    if (!title || weekDays.length === 0) {
      alert('Está faltando o tipo do hábito ou algum dia!')
      return
    }
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <label htmlFor="" className="mt-3 font-semibold">
        Qual a recorrência?
      </label>
      ...
      <div className="mt-3 flex flex-col gap-2">
        {avaiableWeekDays.map((weekDay, index) => (
          ...
          >
            <div className="h-8 w-8 rounded-lg flex items-center justify-center bg-zinc-900 border-2 border-zinc-800 group-data-[state=checked]:bg-green-500 group-data-[state=checked]:border-green-500 transition-colors group-focus:ring-2 focus:ring-violet-500 focus:ring-offset-2 focus:ring-offset-background">
              <Checkbox.Indicator>
